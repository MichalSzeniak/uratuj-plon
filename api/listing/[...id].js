...
export default async function handler(request) {
  try {
    const url = new URL(request.url);
    const pathname = url.pathname;
    // Poprawne wyciąganie ID z dynamicznego segmentu
    const listingId = pathname.split("/").pop() || "default";

    // Upewnij się, że URL w og:url jest poprawny
    const fullUrl = `https://ratujplon.pl${pathname}`;
...
<head>
    ...
    <!-- WAŻNE: Upewnij się, że ścieżka do CSS jest poprawna -->
    <link rel="stylesheet" href="/assets/index.css">
</head>
<body>
    <noscript>You need to enable JavaScript to run this app.</noscript>
    ...
    <!-- USUNIĘTO BLOK SCRIPT Z PRZEKIEROWANIEM -->
    <!-- Zamiast tego, React Router odczyta URL bezpośrednio z paska adresu -->
    
    <!-- WAŻNE: Upewnij się, że ścieżka do JS jest poprawna -->
    <script type="module" src="/assets/index.js"></script>
</body>
</html>`;
